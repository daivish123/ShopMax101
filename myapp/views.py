...
def shop(request):
	...
	return render(request,'shop.html',{'all':all,'product':product,'men':men,'women':women,'kids':kids})

def color_red(request):
	product=Product.objects.filter(product_color='red')
...
def color_blue(request):
	product=Product.objects.filter(product_color='blue')
	all=len(Product.objects.all())
	men=len(Product.objects.filter(product_collection="men"))
	women=len(Product.objects.filter(product_collection="women"))
	kids=len(Product.objects.filter(product_collection="kids"))
...
def collection_men(request):
	product=Product.objects.all()
	all=len(product)
	product=Product.objects.filter(product_collection='men')
	men=len(Product.objects.filter(product_collection="men"))
	women=len(Product.objects.filter(product_collection="women"))
	kids=len(Product.objects.filter(product_collection="kids"))
	return render(request,'shop.html',{'all':all,'product':product,'men':men,'women':women,'kids':kids})
...
def collection_women(request):
	product=Product.objects.all()
	all=len(product)
	product=Product.objects.filter(product_collection='women')
	men=len(Product.objects.filter(product_collection="men"))
	women=len(Product.objects.filter(product_collection="women"))
	kids=len(Product.objects.filter(product_collection="kids"))
	return render(request,'shop.html',{'all':all,'product':product,'men':men,'women':women,'kids':kids})
...
def collection_kids(request):
	product=Product.objects.all()
	all=len(product)
	product=Product.objects.filter(product_collection='kids')
	men=len(Product.objects.filter(product_collection="men"))
	women=len(Product.objects.filter(product_collection="women"))
	kids=len(Product.objects.filter(product_collection="kids"))
	return render(request,'shop.html',{'all':all,'product':product,'men':men,'women':women,'kids':kids})
...
def product_detail(request,pk):
	...
	return render(request,'product_detail.html',{'product':product,'whishlist_flag':whishlist_flag,'cart_flag':cart_flag})
...
def remove_from_cart(request,pk):
	product=Product.objects.get(pk=pk)
	user=User.objects.get(email=request.session['email'])
	carts=Cart.objects.get(user=user,product=product,payment_status="pending")
	carts.delete()
	return redirect('cart')	
